...
# 2. 装平台驱动
codebotd setup-driver           # Linux/macOS 不需 sudo；Windows 需管理员
...
`setup-driver` 自动：
1. 先跑 `codebotd doctor` 检查环境
2. 按平台分支安装 udev 规则 / WinUSB INF / 提示 macOS TCC
3. 给出明确的"下一步"指引
...
```bash
sudo codebotd setup-driver
```

这会：
1. 把 `udev/99-codebot.rules` 复制到 `/etc/udev/rules.d/`
2. `udevadm control --reload-rules && udevadm trigger`
3. 提示 `sudo usermod -aG plugdev $USER`（可选，登录后生效）

如果用 `codebotd setup-driver`（无 sudo）会装到 `~/.config/udev/rules.d/`，
...
这会调 `pnputil /add-driver codebot-inface0.inf /install`。
...
| 设备管理器显示黄色警告 | 卸载设备 → 拔掉 → 重插 → `codebotd setup-driver` |
...
| `USB device scan INFO not found` | 全平台 | 插设备；跑 `codebotd setup-driver` |
